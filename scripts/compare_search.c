...
static keystone_result_t binary_search(const int64_t* arr, size_t n, int64_t key) {
    size_t lo = 0, hi = n;
    while (lo < hi) {
...
static void fill_batch(keystone_batch_item_t* batch, const query_t* queries, size_t num_queries) {
    for (size_t i = 0; i < num_queries; ++i) {
        batch[i].key = queries[i].key;
        batch[i].result = KEYSTONE_NOT_FOUND;
        batch[i].ordinal = i;
    }
}
...
static int run_warmup_pass(const int64_t* data,
    ...
    for (size_t i = 0; i < num_queries; ++i) {
        if (binary_search(data, n, queries[i].key) != KEYSTONE_NOT_FOUND) {
            found++;
        }
    }
    ...
    found = 0;
    for (size_t i = 0; i < num_queries; ++i) {
        if (keystone_search(data, n, queries[i].key, table, 8) != KEYSTONE_NOT_FOUND) {
            found++;
        }
    }
    if (found != expected_found) {
        fprintf(stderr, "warmup KEYSTONE found %zu queries; expected %zu\n", found, expected_found);
        return -1;
    }

    keystone_config_t enhanced_config;
    keystone_config_init(&enhanced_config, KEYSTONE_WORKLOAD_TELEMETRY);
    for (size_t i = 0; i < num_queries; ++i) {
        keystone_search_enhanced(data, n, queries[i].key, table, &enhanced_config);
    }

    fill_batch(batch, queries, num_queries);
    found = keystone_search_parallel(data, n, batch, num_queries, table, 8, config);
    if (found != expected_found) {
    ...
    if (fortran_available) {
        fill_batch(batch, queries, num_queries);
        found = keystone_search_batch_fortran(data, n, batch, num_queries);
        if (found != expected_found) {
...
#if KEYSTONE_BENCH_AUTO
    fill_batch(batch, queries, num_queries);
    found = keystone_search_batch_auto(data, n, batch, num_queries, table, 8, config);
    if (found != expected_found) {
...
int main(void) {
    const char* profile = env_str_or_default("KEYSTONE_PROFILE", "manual");
    const char* bench_mode = env_str_or_default("KEYSTONE_BENCH_MODE", "cold");
    const size_t n = env_size_or_default("KEYSTONE_N", 1000000);
    const size_t num_queries = env_size_or_default("KEYSTONE_QUERIES", 200000);
    const size_t runs = env_size_or_default("KEYSTONE_RUNS", 10);
    const size_t hit_rate_pct = env_percent_or_default("KEYSTONE_HIT_RATE_PCT", 100);
    const size_t data_gap = env_size_or_default("KEYSTONE_DATA_GAP", 1);
    const size_t data_gap_jitter = env_size_or_default("KEYSTONE_DATA_GAP_JITTER", 0);
    const size_t query_stride = env_size_or_default("KEYSTONE_QUERY_STRIDE", 17);
    if (strcmp(bench_mode, "cold") != 0 && strcmp(bench_mode, "warm") != 0) {
        fprintf(stderr, "KEYSTONE_BENCH_MODE must be 'cold' or 'warm'; got '%s'\n", bench_mode);
        return 1;
    }
    const size_t default_warmup_runs = strcmp(bench_mode, "warm") == 0 ? 1 : 0;
    const size_t warmup_runs =
        env_size_allow_zero_or_default("KEYSTONE_WARMUP_RUNS", default_warmup_runs);
    if (data_gap > (size_t)INT64_MAX) {
        fprintf(stderr, "KEYSTONE_DATA_GAP is too large\n");
        return 1;
    }
    if (data_gap_jitter > (size_t)INT64_MAX || data_gap > (size_t)INT64_MAX - data_gap_jitter) {
        fprintf(stderr, "KEYSTONE_DATA_GAP_JITTER is too large\n");
        return 1;
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (data[i - 1] > INT64_MAX - (int64_t)step) {
            fprintf(stderr, "KEYSTONE_DATA_GAP is too large for KEYSTONE_N\n");
            free(data);
            return 1;
        }
    ...
    keystone_batch_item_t* batch = malloc(num_queries * sizeof(keystone_batch_item_t));
    ...
    keystone_parallel_config_t config = {0, 0, 64};
    const int fortran_available = keystone_fortran_backend_available();
    ...
    const size_t batch_bytes = num_queries * sizeof(keystone_batch_item_t);
    ...
    printf("#env=KEYSTONE_PROFILE,KEYSTONE_BENCH_MODE,KEYSTONE_WARMUP_RUNS,KEYSTONE_N,KEYSTONE_QUERIES,KEYSTONE_RUNS,KEYSTONE_HIT_RATE_PCT,KEYSTONE_DATA_GAP,KEYSTONE_DATA_GAP_JITTER,KEYSTONE_QUERY_STRIDE\n");
    ...
    for (size_t run = 1; run <= runs; ++run) {
        ...
        if (classic_found != expected_found) {
            fprintf(stderr, "KEYSTONE found %zu queries; expected %zu\n", classic_found, expected_found);
            return 1;
        }
        ...
        for (size_t i = 0; i < num_queries; ++i) {
            if (keystone_search_enhanced(data, n, queries[i].key, table, &enhanced_config) != KEYSTONE_NOT_FOUND) {
                enhanced_found++;
            }
        }
        end = now_ns();
        if (enhanced_found != expected_found) {
            fprintf(stderr, "enhanced KEYSTONE found %zu queries; expected %zu\n", enhanced_found, expected_found);
            return 1;
        }
        ...
        start = now_ns();
        fill_batch(batch, queries, num_queries);
        size_t found = keystone_search_parallel(data, n, batch, num_queries, table, 8, &config);
        end = now_ns();
        if (found != expected_found) {
