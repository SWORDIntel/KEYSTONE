...
static void test_backend_unavailable_fallback(void) {
    ...
    size_t found = keystone_search_batch_fortran(
        data,
        ARRAY_LEN(data),
        items,
        ARRAY_LEN(items)
    ...
    for (size_t i = 0; i < ARRAY_LEN(items); ++i) {
        assert(items[i].result == KEYSTONE_NOT_FOUND);
...
static void assert_batch_results(const int64_t* data,
                                 ...
                                 keystone_batch_item_t* items,
                                 size_t num_items,
                                 const keystone_result_t* expected,
                                 size_t expected_found) {
    size_t found = keystone_search_batch_fortran(data, n, items, num_items);
...
static void test_fortran_duplicate_data_matches_binary_semantics(void) {
    ...
    keystone_result_t expected[ARRAY_LEN(items)];

    for (size_t i = 0; i < ARRAY_LEN(items); ++i) {
        expected[i] = KEYSTONE_NOT_FOUND;
    }
    expected[0] = 3;
    expected[1] = 3;
    expected[3] = 5;
...
static void test_fortran_sorted_key_fast_path_correctness(void) {
    int64_t data[256];
    keystone_batch_item_t items[128];
    keystone_result_t expected[128];
    ...
    for (size_t i = 0; i < ARRAY_LEN(items); ++i) {
        ...
        if (items[i].key % 2 == 0) {
            expected[i] = (keystone_result_t)(items[i].key / 2);
        } else {
            expected[i] = KEYSTONE_NOT_FOUND;
        }
    }
...
static void test_fortran_batch_matches_scalar(void) {
    ...
    for (size_t i = 0; i < num_items; ++i) {
        keystone_result_t expected = keystone_search(
            data,
            n,
            items[i].key,
            table,
...
static void test_empty_inputs(void) {
    int64_t data[] = {1, 2, 3};
    keystone_batch_item_t item = {.key = 2};

    assert(keystone_search_batch_fortran(data, 0, &item, 1) == 0);
    assert(item.result == KEYSTONE_NOT_FOUND);
    assert(keystone_search_batch_fortran(data, 3, NULL, 1) == 0);
    assert(keystone_search_batch_fortran(NULL, 3, &item, 1) == 0);
    assert(keystone_search_batch_fortran(data, 3, &item, 0) == 0);
}
